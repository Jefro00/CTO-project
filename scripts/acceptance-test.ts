...
import { WorkOrdersService } from '../apps/api/src/work-orders/work-orders.service';
...
import { RemindersService } from '../apps/api/src/reminders/reminders.service';
...
async function runAcceptanceTests() {
  ...
  const jwt = new JwtService({ secret: 'super-secret-jwt-key-for-automotive-os-2026' });
  const ws = new AppWebSocketGateway();
  const auth = new AuthService(db, jwt);
  const locations = new LocationsService(db);
  const users = new UsersService(db);
  const customers = new CustomersService(db);
  const vehicles = new VehiclesService(db);
  const inspections = new InspectionsService(db, ws);
  const media = new MediaService(db, ws);
  const workOrders = new WorkOrdersService(db, ws);
  const documents = new DocumentsService(db, ws);
  const tasks = new TasksService(db, ws);
  const reminders = new RemindersService(db);
  const notifications = new NotificationsService(db);
  const audit = new AuditService(db);
  const backups = new BackupsService(db);
  ...
  try {
    ...
    const rolesList = db.all<any>('SELECT * FROM roles WHERE organization_id = ?', [orgId]);
    const advisorRoleId = rolesList.find((r) => r.name === SystemRole.SERVICE_ADVISOR)?.id;
    const mechanicRoleId = rolesList.find((r) => r.name === SystemRole.MECHANIC)?.id;
    ...
    assert(!!advisorUser.id && advisorUser.role.name === SystemRole.SERVICE_ADVISOR, '3. Создать приемщика (Service Advisor created)');
    ...
    assert(!!mechanicUser.id && mechanicUser.role.name === SystemRole.MECHANIC, '4. Создать механика (Mechanic created)');
    ...
    assert(inspAfterPhotos.media.filter((m: any) => m.type === 'photo').length >= 4, '11. Сделать минимум 4 фотографии (>= 4 inspection photos uploaded)');
    ...
    assert(inspAfterVideo.media.some((m: any) => m.type === 'video'), '12. Снять видео (Inspection video uploaded)');
    ...
    assert(mechanicOrders.data.some((w: any) => w.id === workOrder.id), '18. Увидеть назначенную работу (Mechanic sees assigned order)');
    ...
    assert(
      history.data.some((h) => h.type === 'inspection') &&
      history.data.some((h) => h.type === 'work_order') &&
      history.data.some((h) => h.type === 'media'),
      '22. Увидеть всю историю автомобиля (Unified Timeline aggregates inspections, work orders, media)',
    );
    ...
    assert(!hasPermission(mechanicPerms, 'roles.update'), 'Mechanic cannot edit roles');
    assert(!hasPermission(mechanicPerms, 'backup.create'), 'Mechanic cannot create backup');
    assert(!hasPermission(mechanicPerms, 'audit.read'), 'Mechanic cannot view audit log');
