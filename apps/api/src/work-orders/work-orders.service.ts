...
import { WorkOrderStatus, WorkOrderItemType, InspectionStatus } from '@automotive-os/types';
...
export class WorkOrdersService {
  ...
  getById(orgId: string, id: string) {
    ...
    const documents = this.db.all<any>(
      'SELECT * FROM documents WHERE work_order_id = ? AND deleted_at IS NULL',
      [id],
    );

    const media = this.db.all<any>(
      'SELECT * FROM media WHERE work_order_id = ? AND deleted_at IS NULL',
      [id],
    );

    const tasks = this.db.all<any>(
      'SELECT * FROM tasks WHERE work_order_id = ?',
      [id],
    );
  ...
  create(orgId: string, advisorId: string, userLocationId: string | null, data: any) {
    const id = uuidv4();
    ...
    return this.db.transaction(() => {
      ...
      const countRow = this.db.get<any>(
        'SELECT COUNT(*) as count FROM work_orders WHERE organization_id = ?',
        [orgId],
      );
        ...
        for (const item of data.items) {
          ...
          this.db.run(
            ...
            [
              itemId,
              id,
              item.type || WorkOrderItemType.LABOR,
              item.description,
              q,
              p,
              cost,
      ...
      this.db.run(
        'UPDATE work_orders SET subtotal = ?, total = ? WHERE id = ?',
        [subtotal, total, id],
      );
      ...
      if (data.master_id) {
        this.db.run(
          `INSERT INTO notifications (id, organization_id, user_id, type, title, body, entity_type, entity_id, is_read, created_at)
           VALUES (?, ?, ?, 'work_order.assigned', 'Назначен заказ-наряд', ?, 'work_order', ?, 0, ?)`,
          [uuidv4(), orgId, data.master_id, `Вам назначен заказ-наряд #${number}`, id, now],
        );
  ...
  update(orgId: string, id: string, data: any) {
    ...
    this.db.run(
      ...
      [status, complaint, diagnosis, mileageIn, mileageOut, masterId, discount, now, id, orgId],
    );
  ...
  approve(orgId: string, id: string) {
    ...
    const now = new Date().toISOString();
    this.db.run(
      `UPDATE work_orders SET status = 'approved', updated_at = ? WHERE id = ? AND organization_id = ?`,
      [now, id, orgId],
    );
  ...
  start(orgId: string, id: string) {
    ...
    const now = new Date().toISOString();
    this.db.run(
      `UPDATE work_orders SET status = 'in_progress', updated_at = ? WHERE id = ? AND organization_id = ?`,
      [now, id, orgId],
    );
  ...
  complete(orgId: string, id: string) {
    ...
    const now = new Date().toISOString();
    this.db.run(
      `UPDATE work_orders SET status = 'completed', completed_at = ?, updated_at = ? WHERE id = ? AND organization_id = ?`,
      [now, now, id, orgId],
    );
  ...
  close(orgId: string, id: string) {
    ...
    const now = new Date().toISOString();
    this.db.run(
      `UPDATE work_orders SET status = 'closed', closed_at = ?, updated_at = ? WHERE id = ? AND organization_id = ?`,
      [now, now, id, orgId],
    );
    ...
    this.db.run(
      `INSERT INTO audit_logs (id, organization_id, location_id, user_id, action, entity_type, entity_id, old_values, new_values, ip_address, user_agent, created_at)
       VALUES (?, ?, ?, ?, 'close', 'work_orders', ?, null, '{"status":"closed"}', '127.0.0.1', 'API', ?)`,
      [uuidv4(), orgId, wo.location_id, wo.advisor_id, id, now],
    );
  ...
  // Items
  getItems(orgId: string, workOrderId: string) {
    this.getById(orgId, workOrderId);
    return this.db.all<any>(
      `SELECT i.*, u.first_name as assignee_first_name, u.last_name as assignee_last_name
       FROM work_order_items i
       LEFT JOIN users u ON i.assigned_to = u.id
       WHERE i.work_order_id = ? ORDER BY i.created_at ASC`,
      [workOrderId],
    );
  }

  addItem(orgId: string, workOrderId: string, data: any) {
    this.getById(orgId, workOrderId);
    const id = uuidv4();
    const now = new Date().toISOString();
    ...
    this.db.run(
      ...
      [
        id,
        workOrderId,
        data.type || WorkOrderItemType.LABOR,
        data.description,
        q,
        p,
    ...
    return this.db.get<any>('SELECT * FROM work_order_items WHERE id = ?', [id]);
  }

  updateItem(orgId: string, workOrderId: string, itemId: string, data: any) {
    this.getById(orgId, workOrderId);
    const item = this.db.get<any>('SELECT * FROM work_order_items WHERE id = ? AND work_order_id = ?', [
      itemId,
      workOrderId,
    ]);
    if (!item) {
      throw new NotFoundException({ code: 'ITEM_NOT_FOUND', message: 'Work order item not found' });
    }
    ...
    this.recalculateTotal(workOrderId);
    return this.db.get<any>('SELECT * FROM work_order_items WHERE id = ?', [itemId]);
  }
  ...
  private recalculateTotal(workOrderId: string) {
    const sumRow = this.db.get<any>(
      'SELECT SUM(total_price) as subtotal FROM work_order_items WHERE work_order_id = ?',
      [workOrderId],
    );
    const wo = this.db.get<any>('SELECT discount, tax FROM work_orders WHERE id = ?', [workOrderId]);
