...
let VehiclesService = class VehiclesService {
    ...
    search(orgId, q) {
        const s = `%${q.trim()}%`;
        const rows = this.db.all(`SELECT v.*, c.first_name as customer_first_name, c.last_name as customer_last_name, c.phone as customer_phone
       FROM vehicles v
       JOIN customers c ON v.customer_id = c.id
       WHERE v.organization_id = ? AND v.deleted_at IS NULL
         AND (
       ...
       ORDER BY v.created_at DESC LIMIT 20`, [orgId, s, s, s, s, s, s, s]);
        return rows.map((v) => ({
            ...v,
            customer: {
                id: v.customer_id,
    ...
    getById(orgId, id) {
        const v = this.db.get(`SELECT v.*, c.first_name as customer_first_name, c.last_name as customer_last_name, c.phone as customer_phone, c.email as customer_email, c.address as customer_address
       FROM vehicles v
       JOIN customers c ON v.customer_id = c.id
       WHERE v.id = ? AND v.organization_id = ? AND v.deleted_at IS NULL`, [id, orgId]);
        if (!v) {
    ...
    create(orgId, data) {
        ...
        const existing = this.db.get('SELECT id FROM vehicles WHERE organization_id = ? AND vin = ? AND deleted_at IS NULL', [orgId, data.vin.trim().toUpperCase()]);
        if (existing) {
        ...
        this.db.run(`INSERT INTO vehicles (
    ...
    update(orgId, id, data) {
        ...
        if (data.vin && data.vin.trim().toUpperCase() !== v.vin) {
            const existing = this.db.get('SELECT id FROM vehicles WHERE organization_id = ? AND vin = ? AND id != ? AND deleted_at IS NULL', [orgId, data.vin.trim().toUpperCase(), id]);
            if (existing) {
                throw new common_1.ConflictException({
                    code: 'VIN_ALREADY_EXISTS',
                    message: 'A vehicle with this VIN already exists in the organization',
        ...
        this.db.run(`UPDATE vehicles SET
        customer_id = ?,
        vin = ?,
        license_plate = ?,
        make = ?,
    ...
    delete(orgId, id) {
        ...
        const now = new Date().toISOString();
        this.db.run('UPDATE vehicles SET deleted_at = ?, updated_at = ? WHERE id = ? AND organization_id = ?', [now, now, id, orgId]);
        return { success: true, message: 'Vehicle deleted successfully' };
    }
    // Section 39: Unified vehicle history timeline combining inspection, work_order, document, media, reminder, task
    getHistory(orgId, vehicleId, query) {
        this.getById(orgId, vehicleId);
        const timeline = [];
        // 1. Inspections
        const inspections = this.db.all(`SELECT i.*, u.first_name, u.last_name
       FROM inspections i
       LEFT JOIN users u ON i.created_by = u.id
       WHERE i.vehicle_id = ? AND i.organization_id = ?`, [vehicleId, orgId]);
        ...
        // 2. Work Orders
        const workOrders = this.db.all(`SELECT w.*, u.first_name, u.last_name
       FROM work_orders w
       LEFT JOIN users u ON w.advisor_id = u.id
       WHERE w.vehicle_id = ? AND w.organization_id = ?`, [vehicleId, orgId]);
        for (const wo of workOrders) {
            const items = this.db.all('SELECT * FROM work_order_items WHERE work_order_id = ?', [wo.id]);
            timeline.push({
                id: wo.id,
                type: 'work_order',
        ...
        // 3. Documents
        const documents = this.db.all(`SELECT d.*, u.first_name, u.last_name
       FROM documents d
       LEFT JOIN users u ON d.uploaded_by = u.id
       WHERE d.vehicle_id = ? AND d.organization_id = ? AND d.deleted_at IS NULL`, [vehicleId, orgId]);
        ...
        // 4. Media
        const mediaList = this.db.all(`SELECT m.*, u.first_name, u.last_name
       FROM media m
       LEFT JOIN users u ON m.uploaded_by = u.id
       WHERE m.vehicle_id = ? AND m.organization_id = ? AND m.deleted_at IS NULL`, [vehicleId, orgId]);
        ...
        // 5. Reminders
        const reminders = this.db.all(`SELECT r.*, u.first_name, u.last_name
       FROM reminders r
       LEFT JOIN users u ON r.created_by = u.id
       WHERE r.vehicle_id = ? AND r.organization_id = ?`, [vehicleId, orgId]);
        ...
        // 6. Tasks
        const tasks = this.db.all(`SELECT t.*, u.first_name, u.last_name
       FROM tasks t
       LEFT JOIN users u ON t.created_by = u.id
       WHERE t.vehicle_id = ? AND t.organization_id = ?`, [vehicleId, orgId]);
